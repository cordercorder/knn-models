...
def main(args: Namespace):
    ...
    keys_type_table = {"fp16": np.float16, "fp32": np.float32}
    ...
    merged_keys_type = keys_type_table[args.merged_keys_type]
    merged_datastore_keys = np.memmap(
        merged_datastore_keys_path,
        dtype=merged_keys_type,
        mode="w+",
    ...
    prev_idx = 0
    for datastore, datastore_size, keys_type in zip(args.datastore_list, args.datastore_size_list, args.keys_type_list):
        ...
        datastore_keys = np.memmap(
            datastore_keys_path,
            dtype=keys_type_table[keys_type],
            mode="r",
            shape=(datastore_size, args.keys_dimension)
        )
        merged_datastore_keys[prev_idx: current_idx] = datastore_keys.astype(merged_keys_type)
...
def get_parser():
    ...
    parser.add_argument("--keys-type-list", type=str, nargs="+", choices=["fp16", "fp32"], required=True, help="space separated datastore keys dtype")
    ...
    parser.add_argument("--merged-keys-type", type=str, choices=["fp16", "fp32"], default="fp16", help="keys dtype of the merged datastore")
    return parser


def validate_args(args: Namespace):
    assert len(args.datastore_list) == len(args.datastore_size_list) == len(args.keys_type_list), \
        f"The length of `datastore_list`, `datastore_size_list` and `keys_type_list` must be the same." \
        f"Length of `datastore_list`: {args.datastore_list}, length of `datastore_size_list`: {args.datastore_size_list}, " \
        f"length of `keys_type_list`: {args.keys_type_list}."
